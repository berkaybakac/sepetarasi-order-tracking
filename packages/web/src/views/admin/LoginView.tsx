...
export function LoginView() {
	...
	const handleLogin = async (e: React.FormEvent) => {
		...
		try {
			...
			setError(err.message || "Giriş başarısız. Parolanızı kontrol edin.");
		} finally {
			setIsLoading(false);
		}
	};
		...
		<div className="min-h-screen bg-slate-900 flex items-center justify-center p-4 relative overflow-hidden">
			...
			<div className="w-full max-w-md bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-2xl relative z-10 transition-all duration-300 hover:shadow-blue-500/10 hover:border-white/20">
				<div className="text-center mb-8">
					...
					<p className="text-slate-400 mt-2 text-sm">Sepetarası panelini yönetmek için giriş yapın</p>
				</div>

				<form onSubmit={handleLogin} className="space-y-6">
					<div>
						<label className="block text-sm font-medium text-slate-300 mb-2">Yönetici Parolası</label>
					...
					{error && (
						<div className="bg-brand-danger/10 border border-brand-danger/20 text-brand-danger text-sm rounded-xl p-3 flex items-start gap-2">
