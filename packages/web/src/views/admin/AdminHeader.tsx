			...
			<header className="bg-slate-900/50 backdrop-blur-md border-b border-white/5 py-4 px-6 sticky top-0 z-40 transition-all">
				<div className="flex items-center justify-between max-w-7xl mx-auto">
					<div className="flex items-center gap-3">
						...
						<span className={`ml-2 w-2.5 h-2.5 rounded-full ${connected ? "bg-brand-success shadow-[0_0_12px_rgba(16,185,129,0.8)] animate-pulse" : "bg-brand-danger shadow-[0_0_12px_rgba(239,68,68,0.8)]"}`} title={connected ? "Bağlı" : "Bağlantı Koptu"} />
					</div>
					...
					<div className="bg-slate-900/90 backdrop-blur-xl border border-white/10 rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
						<div className="px-6 py-5 border-b border-white/5 flex justify-between items-center">
							<h3 className="font-semibold text-white text-lg flex items-center gap-2">
								<svg className="w-5 h-5 text-brand-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg>
								Güvenlik Ayarları
							</h3>
							<button onClick={() => setShowModal(false)} className="text-slate-400 hover:text-white transition-colors p-1 bg-white/5 hover:bg-white/10 rounded-lg">
						...
						<form onSubmit={handleChangePassword} className="p-6 space-y-5">
							{message && (
								<div className={`p-4 rounded-xl text-sm flex items-start gap-2 ${message.type === 'error' ? 'bg-brand-danger/10 text-brand-danger border border-brand-danger/20' : 'bg-brand-success/10 text-brand-success border border-brand-success/20'}`}>
							...
							<div>
								<label className="block text-sm font-medium text-slate-300 mb-2">Mevcut Parola</label>
								<input 
									type="password" 
							...
							<div>
								<label className="block text-sm font-medium text-slate-300 mb-2">Yeni Parola</label>
								<input 
									type="password" 
									value={newPassword}
